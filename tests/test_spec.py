...
def test_every_spec_declares_what_the_loader_requires(provider_id):
    ...
    # The threshold is proportional, not a constant: the refresh window was
    # measured at 3.55 days on macOS/Max 5x and 28.3 days on Linux/Team, so a
    # fixed number of days is wrong under one regime or the other.
    assert isinstance(definition["policy"]["warn_days_floor"], (int, float))
    assert definition["policy"]["warn_days_floor"] > 0
