...
class ClaudeProvider:
    ...
    def warn_days(self) -> float:
        """Floor only. The real threshold is proportional -- see :meth:`liveness`."""
        return float(self.spec["policy"].get("warn_days_floor", 1))

    @property
    def warn_fraction(self) -> float:
        return float(self.spec["policy"].get("warn_fraction", 0.25))
    ...
    def liveness(self, blob: bytes | None, *, now_ms: int) -> Liveness:
        ...
        if expires <= now_ms:
            return Liveness(CLOSED, expires_at_ms=expires, days_left=days)

        threshold = self._warn_threshold_days(_parse(blob))
        state = CLOSING if days <= threshold else LIVE
        return Liveness(state, expires_at_ms=expires, days_left=days)

    def _warn_threshold_days(self, data: dict) -> float:
        """How little is "closing soon", for *this* credential.

        A fixed number of days cannot work. The refresh window was measured at
        3.55 days on macOS/Max 5x and 28.3 days on Linux/Team, so any constant
        is either permanently amber under one regime or silent under the other.

        The width is recoverable from the blob: both timestamps are minted in
        one server response, so the gap between them plus the access-token
        lifetime is the window. The gap alone is a good enough proxy, and using
        it means the threshold tracks whatever the server is currently issuing
        without this code knowing which regime it is in.
        """
        oauth = data.get("claudeAiOauth")
        floor = self.warn_days
        if not isinstance(oauth, dict):
            return floor
        access = oauth.get("expiresAt")
        refresh = oauth.get("refreshTokenExpiresAt")
        if not isinstance(access, (int, float)) or not isinstance(refresh, (int, float)):
            return floor
        span_days = (refresh - access) / MS_PER_DAY
        return max(floor, span_days * self.warn_fraction)
